...
| Video-only stream | Fresh OBS connection negotiates one H.264 track and no audio track; no mic toggle on Host/Remote or microphone permission prompt |
| Video latency comparison | Same quality, connection and OBS settings: film a visible millisecond timer and compare delay against the previous build; do not assume removing audio fixes it |
...
| Video reconnect recovery | Repeat OBS Properties OK/reconnect; video resumes without accumulating delay, with Remote Preview OFF and ON |
